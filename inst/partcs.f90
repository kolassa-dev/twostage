!######################################
     integer function partcs(m1,n1,m2,n2,perm,parts,nparts)
implicit none
     integer m1,n1,m2,n2,perm(m1+n1+m2+n2),aa,ii,jj,kk,ufi,nparts,ll,bb,gg
                    ...
                    if(ll.ne.jj) then
                       do gg=1,m1+m2
                          if((gg.ne.ii).and.(gg.ne.kk)) then
                             lead=imat(ii,jj)*imat(kk,jj)*imat(gg,ll)
!                            write(6,*) "lead",lead
                             partcs    =partcs    +lead
                             ux(ii)=.false.;ux(kk)=.false.;ux(gg)=.false.;
                             uy(ll)=.false.;uy(jj)=.false.
!    write(6,*) "ux",ux,"uy",uy
                             partx(1)=partx(1)+lead*ufi(m1+m2,n1+n2,imat,ux,uy)
                             ux(ii)=.true.;ux(kk)=.true.;ux(gg)=.true.
                             uy(ll)=.true.;uy(jj)=.true.
                             do aa=1,m1+m2
                             ...
                             partx(5)=partx(5)+lead*(imat(gg,jj)+imat(ii,ll)+imat(kk,ll))
                          end if!gg
                       end do!gg
                    end if!ll
                 end do!ll
...
!    write(6,*) "ux",ux,"uy",uy
     partcs    =partcs    *ufi(m1+m2,n1+n2,imat,ux,uy)
     do aa=1,nparts
        parts(aa)=parts(aa)+partx(aa)
        monster(2)=monster(2)+partx(aa)
     end do
     if(partcs    .ne.monster(2)) then
        write(6,*) "parte partcs    ",partcs    ,"monster(2)",monster(2),"This many parts",nparts,"These parts",partx
     end if
